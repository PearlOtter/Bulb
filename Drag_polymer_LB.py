...
N_S = int(768/4) # int(nL3*((X/L)*((H-2*WALL_OFFSET)/L)*(Z/L)))
m_R = DENSITY * Vol_P_R
...
lbf = espressomd.lb.LBFluidGPU(kT = 0.0*kT, seed = Seed,
                            agrid = AGRID, dens = DENSITY, visc = KINEMATIC_VISCOSITY,
...
for i in range(2000):
    ...
    ################################################################
    ############ Running shear flow for LD simulation ##############
    system.integrator.run(100)
    ###############################################################
    ###############################################################
    
    ################################################################
    ############ Shear flow making for LD simulation ###############
    # for h in range(100):
